...
with server_config do # Chef server context, so we can get the databag.
  ...
  unless publish_info.empty?
    ...
    bash 'Publishing tar file' do
      code <<-EOH
        scp -i #{workflow_workspace_repo}/ssh_key #{workflow_workspace_repo}/#{build_file} #{publish_info['user']}@#{publish_info['host']}:~/
        ssh -i #{workflow_workspace_repo}/ssh_key #{publish_info['user']}@#{publish_info['host']} "#{sudo} mv ~/#{build_file} #{publish_info['dir']}/
        EOH
      action :run
    end
  end
